class read_xtn extends uvm_sequence_item;
	...
	// read (type bit) 
	 ...
	 bit[7:0] header;   //[1:0]addresss 
	 bit[7:0] payload_data[];  //header will decide the payload length from header[7:2]
	 bit [7:0]parity;
...
/*function void read_xtn::post_randomize(); //to calculate parity
    parity=0^header;
        foreach(payload[i])
              begin
